...
export function licenseAccessEndsAt(row) {
  ...
  const hasPaidSubscription = ['active', 'expiring_soon'].includes(status);

  if (license.type === 'subscription' && hasFutureTrial && PAYMENT_BLOCKED_STATUSES.has(status)) {
...
export function licenseForPaymentResponse(subscriptionLicense, activeTrialLicense) {
  const subscriptionStatus = String(subscriptionLicense?.status || '').toLowerCase();
  if (activeTrialLicense && PAYMENT_BLOCKED_STATUSES.has(subscriptionStatus)) {
    return activeTrialLicense;
  }
  return subscriptionLicense || activeTrialLicense || null;
}

function effectivePublicStatus(license) {
  const status = String(license?.status || '').toLowerCase();
  if (license?.type !== 'subscription' || !PAYMENT_BLOCKED_STATUSES.has(status)) {
    return status;
  }
  const trialEndValue = license.trial_ends_at || license.related_trial_ends_at || null;
  const trialEnd = trialEndValue ? new Date(trialEndValue) : null;
  if (trialEnd && Number.isFinite(trialEnd.getTime()) && trialEnd > new Date()) {
    return 'trialing';
  }
  return status;
}
...
export function publicLicenseResponse(row, message = '') {
  ...
  const status = effectivePublicStatus(license);
  return {
    status,
    type: license.type,
    ...
    email: license.email,
    seats: license.seats,
    message: message || (ACTIVE_STATUSES.has(status) ? 'License active' : 'License not active'),
  };
}
